...
Requirements: Ruby ≥ 3.2, Go (version in `tools/rib2origin/go.mod`; stdlib only, compiled on first run), `jq` recommended (streams the ~69MB ipverse JSON; a stdlib fallback exists but is memory-hungry), `gh` CLI for publishing.
